...
fn test_buy_back_is_permissionless() {
    ...
    assert!(dispatcher.get_position_token_id(sell_token) != 0, "Buy back should work for any user");
}
...
fn test_claim_proceeds_rejects_when_orders_not_ended() {
    ...
    // Before the order ends, the claim is rejected.
    dispatcher
        .claim_order(
            OrderKey {
                sell_token: sell_token,
...
fn test_claiming_one_token_does_not_affect_another() {
    ...
    // Verify both tokens have positions
    assert!(dispatcher.get_position_token_id(sell_token_1) != 0, "Token1 has a position");
    assert!(dispatcher.get_position_token_id(sell_token_2) != 0, "Token2 has a position");

    // Fast forward past first order
    start_cheat_block_timestamp_global(end_time1 + 1);

    // Claim token 1 by its key.
    mock_call(mock_positions, selector!("withdraw_proceeds_from_sale_to"), 500_u128, 1);
    ...
    assert!(claimed == 500, "Token1 claims independently");

    // Token 2's order has not matured, so its key is still rejected -- the two
    // tokens share no queue and no state.
    assert!(dispatcher.get_position_token_id(sell_token_2) != 0, "Token2 order still open");
}
...
fn test_fuzz_buy_back_with_valid_durations(duration_offset: u64) {
    ...
    assert!(dispatcher.get_position_token_id(sell_token) != 0, "Order should be created");
}
...
fn test_fuzz_minimum_amount_validation(amount_raw: u128) {
    ...
    assert!(
        dispatcher.get_position_token_id(sell_token) != 0, "Order should be created at minimum",
    );
}
...
fn test_multiple_orders_reuse_position() {
    ...
    assert!(dispatcher.get_position_token_id(sell_token) != 0, "Position created");
    ...
    assert!(dispatcher.get_position_token_id(sell_token) != 0, "Position reused");
...
fn test_claim_fails_when_no_position() {
    ...
    dispatcher
        .claim_order(
            OrderKey {
                sell_token, buy_token: buyback_token, fee: DEFAULT_FEE, start_time: 0, end_time: 1,
            },
        );
}
...
fn test_claim_fails_when_no_completed_orders() {
    ...
    // The order has not reached end_time, so the claim is rejected.
    dispatcher
        .claim_order(
            OrderKey {
                sell_token, buy_token: buyback_token, fee: DEFAULT_FEE, start_time: 0, end_time,
            },
        );
}
