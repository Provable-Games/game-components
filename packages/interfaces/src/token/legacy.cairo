// Legacy token interface — the ORIGINAL multi-game minigame token (separate
// token contract, registry-backed, mutable token state synced via
// `update_game`). Kept for deployed denshokan; the minigame token STANDARD is
// now the single-game, self-bound trait in `token/core.cairo`
// (`IMinigameToken`).
...
///
/// FROZEN VALUE: deployed denshokan contracts register this id on-chain under
/// its original derivation, from when this trait was named `IMinigameToken`.
/// The standard NAME (`IMINIGAME_TOKEN_ID`) moved to the new self-bound token,
/// but this value must never change — it identifies the legacy surface that
/// live contracts advertise.
pub const IMINIGAME_TOKEN_LEGACY_ID: felt252 =
    0x246f614bd76b91c378a91877851f2ccdb99278e9fb77c782a22355059ce9906;

#[starknet::interface]
pub trait IMinigameTokenLegacy<TState> {
