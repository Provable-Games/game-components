...
use crate::tests::support::test_common::erc721::ERC721SpyHelpers;
use crate::tests::support::test_common::mocks::erc721::{
    DualCaseERC721Mock, SnakeERC721MockWithHooks,
};
use crate::tests::support::testing as utils;
use crate::tests::support::testing::constants::{
    BASE_URI, BASE_URI_2, CALLER, DATA, NAME, OPERATOR, OTHER, OWNER, PUBKEY, RECIPIENT, SPENDER,
    SYMBOL, TOKEN_ID, TOKEN_ID_2, ZERO,
};
use crate::tests::support::testing::{
    EventSpyExt, EventSpyQueue as EventSpy, ExpectedEvent, spy_events,
};
...
fn test_safe_transfer_from_to_non_receiver() {
    super::super::receiver_boundaries::nonreceiver_case(0);
}

#[test]
#[feature("safe_dispatcher")]
fn test_safeTransferFrom_to_non_receiver() {
    super::super::receiver_boundaries::nonreceiver_case(1);
}
...
fn test__safe_mint_to_non_receiver() {
    super::super::receiver_boundaries::nonreceiver_case(2);
}
...
fn test_burn() {
    ...
    assert_eq!(state.ERC721_owners.read(TOKEN_ID.try_into().unwrap()), ZERO);
    assert_eq!(state.balance_of(OWNER), 0);
    assert_eq!(state.ERC721_token_approvals.read(TOKEN_ID.try_into().unwrap()), ZERO);
}
...
fn test__exists() {
    ...
    let mut owner = state.ERC721_owners.read(token_id.try_into().unwrap());
    ...
    owner = state.ERC721_owners.read(token_id.try_into().unwrap());
