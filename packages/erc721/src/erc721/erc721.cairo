...
// OpenZeppelin Contracts for Cairo v4.0.1 (token/src/erc721/erc721.cairo)
...
pub mod ERC721Component {
    ...
    pub struct Storage {
        ...
        pub ERC721_owners: Map<felt252, ContractAddress>,
        pub ERC721_balances: Map<ContractAddress, felt252>,
        pub ERC721_token_approvals: Map<felt252, ContractAddress>,
    ...
    pub mod Errors {
        ...
        pub const BALANCE_OVERFLOW: felt252 = 'ERC721: balance overflow';
        pub const BALANCE_UNDERFLOW: felt252 = 'u256_sub Overflow';
    ...
    fn token_key(token_id: u256) -> felt252 {
        let Some(key) = token_id.try_into() else {
            core::panic_with_felt252(Errors::INVALID_TOKEN_ID);
        };
        key
    }
    ...
    pub trait ERC721TokenOwnerTrait<TContractState> {
        ...
        fn owner_of(
            ...
            self.ERC721_owners.read(key)
        }
    }

    pub impl ConsecutiveERC721TokenOwnerImpl<
        TContractState,
        ...
        +Drop<TContractState>,
    > of ERC721TokenOwnerTrait<TContractState> {
        /// See `ERC721TokenOwnerTrait::owner_of`. Implementation that checks the sequential
        /// ownership structure for tokens that have been minted as part of a batch, and not yet
        /// transferred.
        fn owner_of(self: @ComponentState<TContractState>, token_id: u256) -> ContractAddress {
            let Some(key) = token_id.try_into() else {
                return Zero::zero();
            };
            let owner = self.ERC721_owners.read(key);
    ...
    > of interface::IERC721<ComponentState<TContractState>> {
        ...
        fn get_approved(self: @ComponentState<TContractState>, token_id: u256) -> ContractAddress {
            self._require_owned(token_id);
            self.ERC721_token_approvals.read(token_key(token_id))
        }
    ...
    pub impl InternalImpl<
        ...
        impl OwnerOf: ERC721TokenOwnerTrait<TContractState>,
        +Drop<TContractState>,
    > of InternalTrait<TContractState> {
        ...
        /// `ERC721TokenOwnerTrait` custom implementation.
        ///
        /// NOTE: The increment is limited to `u128::MAX`; the aggregate balance can be
        /// any felt252 integer in 0..P. Reject sums >= P rather than wrapping modulo P.
        ///
        /// WARNING: Increasing an account's balance using this function tends to be paired with
        /// an custom implementation of the `ERC721TokenOwnerTrait` trait to resolve the ownership
        /// of the corresponding tokens so that balances and ownership remain consistent with one
        /// another.
        fn increase_balance(
            ref self: ComponentState<TContractState>, account: ContractAddress, value: u128,
        ) {
            let current_balance = self.ERC721_balances.read(account);
            let next_balance = current_balance + value.into();
            // current < P and value <= u128::MAX < P imply the integer sum < 2P.
            // A wrapped result is strictly less than value, hence fits u128.
            // A nonwrapped result is >= value; a wide result therefore cannot wrap.
            let narrow: Option<u128> = next_balance.try_into();
            if let Some(next) = narrow {
                assert(next >= value, Errors::BALANCE_OVERFLOW);
            }
            self.ERC721_balances.write(account, next_balance);
        }
        ...
        fn update(
            ...
            if !from.is_zero() {
                ...
                let balance = self.ERC721_balances.read(from);
                assert(balance != 0, Errors::BALANCE_UNDERFLOW);
                self.ERC721_balances.write(from, balance - 1);
            }
            if !to.is_zero() {
                let next_balance = self.ERC721_balances.read(to) + 1;
                // Adding one wraps modulo P only when the result is zero.
                assert(next_balance != 0, Errors::BALANCE_OVERFLOW);
                self.ERC721_balances.write(to, next_balance);
            }

            self.ERC721_owners.write(token_key(token_id), to);
        ...
        fn _owner_of(self: @ComponentState<TContractState>, token_id: u256) -> ContractAddress {
            // The supported token domain applies even to custom ownership implementations.
            // Preserve delegation for every representable ID; update still invokes its
            // before hook before ownership lookup and authorization.
            let key: Option<felt252> = token_id.try_into();
            if key.is_none() {
                return Zero::zero();
            }
        ...
        fn _approve_with_optional_event(
            ...
            self.ERC721_token_approvals.write(token_key(token_id), to);
        }
...
/// Implementation of ERC721TokenOwnerTrait for the basic (non-consecutive) ERC721 component.
