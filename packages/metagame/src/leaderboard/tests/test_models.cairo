...
fn test_leaderboard_creation_empty_token_ids() {
    let leaderboard = Leaderboard { context_id: 1, token_ids: array![] };

    assert!(leaderboard.context_id == 1, "Context ID should be 1");
    assert!(leaderboard.token_ids.len() == 0, "Token IDs should be empty");
}

#[test]
fn test_leaderboard_creation_with_single_token() {
    let leaderboard = Leaderboard { context_id: 42, token_ids: array![100] };

    assert!(leaderboard.context_id == 42, "Context ID should be 42");
...
fn test_leaderboard_creation_with_multiple_tokens() {
    let leaderboard = Leaderboard { context_id: 1, token_ids: array![1, 2, 3, 4, 5] };
...
fn test_leaderboard_creation_with_zero_tournament_id() {
    let leaderboard = Leaderboard { context_id: 0, token_ids: array![1] };

    assert!(leaderboard.context_id == 0, "Context ID should be 0");
}
...
fn test_leaderboard_tournament_id_access_max() {
    let leaderboard = Leaderboard { context_id: MAX_U64, token_ids: array![] };

    assert!(leaderboard.context_id == MAX_U64, "Context ID should be MAX_U64");
}

#[test]
fn test_leaderboard_token_ids_access() {
    let leaderboard = Leaderboard { context_id: 1, token_ids: array![10, 20, 30] };
...
fn test_leaderboard_token_ids_max_values() {
    ...
    let leaderboard = Leaderboard { context_id: 1, token_ids: array![max_id, max_id_minus_1] };
...
fn test_leaderboard_serialize_empty() {
    ...
    // Should serialize: context_id (2 felt252 for u64) + array length (1 felt252) = 3 elements
    // min
    assert!(output.len() >= 2, "Should have at least context_id and array length");
}

#[test]
fn test_leaderboard_serialize_with_tokens() {
    let leaderboard = Leaderboard { context_id: 5, token_ids: array![1, 2, 3] };

    let mut output = array![];
    leaderboard.serialize(ref output);

    // Should contain context_id, array length, and token values
...
fn test_leaderboard_serialize_max_values() {
    let max_id: felt252 = MAX_U64.into();
    let leaderboard = Leaderboard { context_id: MAX_U64, token_ids: array![max_id] };
...
fn test_leaderboard_deserialize_empty() {
    let original = Leaderboard { context_id: 1, token_ids: array![] };
    ...
    assert!(deserialized.context_id == 1, "Context ID should match");
    assert!(deserialized.token_ids.len() == 0, "Token IDs should be empty");
}

#[test]
fn test_leaderboard_deserialize_with_tokens() {
    let original = Leaderboard { context_id: 42, token_ids: array![10, 20, 30] };
    ...
    assert!(deserialized.context_id == 42, "Context ID should match");
...
fn test_leaderboard_roundtrip_serialization() {
    ...
    let configs: Array<Leaderboard> = array![
        Leaderboard { context_id: 0, token_ids: array![] },
        Leaderboard { context_id: 1, token_ids: array![1] },
        Leaderboard { context_id: 100, token_ids: array![1, 2, 3, 4, 5] },
        Leaderboard { context_id: MAX_U64, token_ids: array![max_id] },
    ];
    ...
    loop {
        ...
        assert!(deserialized.context_id == *original.context_id, "Context ID mismatch");
        assert!(
...
#[test]
#[fuzzer]
fn test_fuzz_leaderboard_creation(context_id: u64) {
    // Fuzz test: struct creation never panics with random context_id
    let leaderboard = Leaderboard { context_id, token_ids: array![] };
    assert!(leaderboard.context_id == context_id, "Context ID should match");
}

#[test]
#[fuzzer]
fn test_fuzz_leaderboard_serialization_roundtrip(context_id: u64) {
    // Fuzz test: serialization roundtrip always preserves data
    let original = Leaderboard { context_id, token_ids: array![1, 2, 3] };
    ...
    assert!(deserialized.context_id == context_id, "Context ID should be preserved");
...
fn test_leaderboard_creation_large_array() {
    ...
    let leaderboard = Leaderboard { context_id: 1, token_ids };
