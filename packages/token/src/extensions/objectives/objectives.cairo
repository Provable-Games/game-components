...
pub mod ObjectivesComponent {
    ...
    use crate::extensions::objectives::interface::{IMinigameTokenObjectives, TokenObjective, IMINIGAME_TOKEN_OBJECTIVES_ID};
    use crate::examples::multi_game_contract::{IMinigameRegistryDispatcher, IMinigameRegistryDispatcherTrait};
    ...
    > of IMinigameTokenObjectives<ComponentState<TContractState>> {
        ...
        fn all_objectives_completed(self: @ComponentState<TContractState>, token_id: u64) -> bool {
            ...
            while index < total_count {
                let objective: TokenObjective = self.token_objectives.entry(token_id).entry(index).read();
        ...
        // this shouldn't be an exposed enpoint as anyone could call it and create an objective (without verification)
        fn create_objective(
            ...
            let minigame_token_dispatcher = IMinigameTokenDispatcher { contract_address: game_address };
            ...
            let game_metadata_opt = game_registry_dispatcher.game_metadata(game_id);
            ...
            assert!(
                ...
                "Denshokan: Objectives address {} not registered by caller",
                objectives_address_display,
            );
            let objective_data_json = create_objectives_json(array![objective_data].span());
            self.emit(ObjectiveCreated { game_address, objective_id, objective_data: objective_data_json });
        }
    }
    ...
    > of OptionalObjectives<TContractState> {
        fn validate_objectives(
            ...
            match objective_ids {
                Option::Some(objective_ids) => {
                    ...
                    assert!(supports_objectives, "MinigameToken: Contract does not support IMinigameTokenObjectives");
                    ...
                    if !objectives_address.is_zero() {
                        ...
                        loop {
                            ...
                            assert!(
                                ...
                                "CoreToken: Objective ID does not exist"
                            );
                            index += 1;
                        }
                    }
                    
                    (objective_ids.len().try_into().unwrap(), Option::Some(objective_ids))
                },
        ...
        fn update_objectives(ref self: TContractState, token_id: u64, game_address: ContractAddress, objectives_count: u32) -> bool {
            ...
            let minigame_dispatcher = IMinigameDispatcher { contract_address: game_address };
            let objectives_address = minigame_dispatcher.objectives_address();
            let game_objectives_dispatcher = IMinigameObjectivesDispatcher { contract_address: objectives_address };
            let total_count = objectives_count;
            let mut index = 0;
            let mut completed_count = 0;

            while index < total_count {
                let objective: TokenObjective = component.token_objectives.entry(token_id).entry(index).read();
                let is_objective_completed = game_objectives_dispatcher.completed_objective(token_id, objective.objective_id);
                if is_objective_completed && !objective.completed {
                    component.token_objectives.entry(token_id).entry(index).write(TokenObjective {
