...
Defined in `packages/interfaces/src/token/core.cairo`. The no-arg
`initializer()` registers `IMINIGAME_TOKEN_ID` plus the absorbed minter's
`IMINIGAME_TOKEN_MINTER_ID` — and nothing else: SRC5 is honest, a standard
token does not implement `IMinigameTokenLegacy` and does not advertise the
legacy id. Consumers branch on `IMINIGAME_TOKEN_ID` instead of resolving
...
Requires: `ERC721Component`, `SRC5Component`, and an `ERC721HooksTrait`
