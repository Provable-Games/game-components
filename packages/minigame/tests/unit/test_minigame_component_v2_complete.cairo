...
// Tests MC-U2 to MC-U5 are skipped because the current implementation
...
fn test_mc_u10_pre_action_with_non_existent_token() {
    ...
    // Mock owner_of to return zero address (non-existent)
    start_mock_call::<ContractAddress>(
        token_address,
        selector!("owner_of"),
...
fn test_mc_u13_post_action_with_valid_token() {
    ...
    // Mock update_game to succeed
    start_mock_call::<()>(
        token_address,
        selector!("update_game"),
        ()
    );
    ...
    // Clean up
    stop_mock_call(token_address, selector!("update_game"));
}
...
fn test_mc_u14_assert_token_ownership_with_owned_token() {
    ...
    // Mock owner_of to return get_caller_address() which will be the test contract
    // We can't control get_caller_address() but we can make owner_of return the same
    ...
    // Assert ownership - this should pass since owner_of returns get_caller_address()
    game_components_minigame::libs::assert_token_ownership(token_address, 1);
